...
const Footer = () => {
    ...
    return (
        <footer className="mx-6 bg-gray-800 text-white">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-gray-700 text-white">
                    <div>
                        <Link href="/" className="flex items-center gap-3">
                            <SquareLogo size={56} className="shrink-0 logo-glow" rounded={false} />
                        </Link>
                        <p className="max-w-[410px] mt-6 text-sm text-white">Welcome to Zillion, your ultimate destination for the latest and smartest gadgets. From smartphones and smartwatches to essential accessories, we bring you the best in innovation — all in one place.</p>
                        <div className="flex items-center gap-3 mt-5">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} className="flex items-center justify-center w-10 h-10 bg-gray-700 hover:scale-105 hover:border border-gray-600 transition rounded-full text-white">
                                    <item.icon />
                                </Link>
                            ))}
                        </div>
                    ...
                    <div className="flex flex-wrap justify-between w-full md:w-[45%] gap-5 text-sm">
                        {linkSections.map((section, index) => (
                            <div key={index}>
                                <h3 className="font-medium text-white md:mb-5 mb-3">{section.title}</h3>
                                <ul className="space-y-2.5">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2">
                                            {link.icon && <link.icon />}
                                            <Link href={link.path} className="text-white hover:underline transition">{link.text}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                <p className="py-4 text-sm text-white">
                    Copyright 2025 © Zillion All Right Reserved.
                </p>
            </div>
        </footer>
    );
